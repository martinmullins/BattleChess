/*
 * fuzz_next_slot_bwd.c — FUN_1000_db65
 *
 * Scans backward (mod 8) through a circular 8-entry slot table, skipping entries
 * whose occupancy byte (at offset +0x0f within each 0x10-byte entry) equals -1.
 * Returns the index of the first occupied slot found before the start.
 *
 * Invariants:
 ...
 *
 * Build:
 *   clang -fsanitize=fuzzer,address,undefined -O1 -o fuzz_next_slot_bwd \
 *         fuzz_next_slot_bwd.c tier3_impl.c seg_storage.c
 */
#include <stdint.h>
#include <stddef.h>
#include <string.h>
...
#define SLOT_BASE  0x2000
#define SLOT_OFFY  0x0f
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    int result = FUN_1000_db65(SLOT_BASE, start);
